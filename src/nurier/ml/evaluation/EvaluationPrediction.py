...
class EvaluationPrediction:
    ...
    def __init__(self, model: FDSModel):
        ...
        self.label = ML_Field.get_label()

        self.prediction = model.get_prediction()
        self.prediction_result = model.prediction_result()
    ...
    def evaluation_model_result(self) -> list:
        ...
        if self.model.media_group_name is not None:
            result.append(f"###              Media Type : {self.model.media_group_name}")
        if self.model.media_group_code_array is not None:
            result.append(f"###              Media Code List : {self.model.media_group_code_array}")
        result.append("###")
        result.append("##############################################")
        result.append("")
        result.append("")
        ...
        for predictions_row in self.prediction.filter("prediction==1.0").filter(f"{self.label}==1.0")._jdf.showString(20, 50, False).split("\n"):
            result.append(predictions_row)
