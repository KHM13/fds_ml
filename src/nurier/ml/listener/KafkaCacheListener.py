...
class KafkaCacheListener:
    ...
    async def start_listener(self, ib_schedule, sb_schedule):
        await self._consumer.start()
        while self.is_loop:
            await self._start_consumer(ib_schedule, sb_schedule)

    async def _start_consumer(self, ib_schedule, sb_schedule):
        try:
            #consume_message = await self._consumer.getone()
            consumedata = self._consumer.getmany(timeout_ms=5000, max_records=10)
            for topic_info, con_message in consumedata.items():
                print(f"topic info : {topic_info}")
                print(f"message size : {len(con_message)}")
                for consume_message in con_message:
                    print(f"consumer value : {consume_message.value}")
                    self.setting_event(consume_message.value, ib_schedule, sb_schedule)
