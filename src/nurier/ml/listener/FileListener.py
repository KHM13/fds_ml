...
from src.nurier.ml.prediction.FDSPredictionSchedule import FDSPredictionSchedule
...
class FileListener:
    ...
    async def read_file(self, ib_schedule, sb_schedule):
        async with aiofiles.open(f"{prop().get_original_data_file_path()}ML_ALL_TEST_1.csv", "r", encoding="EUC-KR") as f:
            async for read_line in AsyncDictReader(f):
                event = PredictionEvent(self.set_message_value(read_line))
                row_data = MessageHandler().get_dataframe_from_message(event)
                event.row_message = row_data
                if ib_schedule.prediction_model.model_IB.is_use_media_code(event.message.EBNK_MED_DSC):
                    ...
                    ib_schedule.use_data_IB(True, event)
        if len(ib_schedule.row_list_IB) == 0:
            ib_schedule.is_loop = False
        else:
            await ib_schedule.run()
            ib_schedule.is_loop = False
        if len(sb_schedule.row_list_SB) == 0:
            sb_schedule.is_loop = False
        else:
            await sb_schedule.run()
            sb_schedule.is_loop = False
    ...
    def set_message_value(self, consume_value: dict):
        ...
        try:
            ...
            if consume_value.get("totalScore") is not None and scutil().isNumberic(consume_value.get("totalScore")):
                message.totalScore = float(consume_value.get("totalScore"))
            else:
                message.totalScore = 0.0
