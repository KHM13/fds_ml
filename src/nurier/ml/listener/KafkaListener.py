...
class KafkaListener:
    ...
    def __init__(self):
        ...
        self._consumer = KafkaConsumer(
            ...
            fetch_max_wait_ms=prop().get_prediction_schedule_period_ms(),
            max_poll_records=prop().get_prediction_schedule_datarow_max(),
        )
    ...
    def _start_consumer(self, ib_schedule, sb_schedule):
        try:
            consumedata = self._consumer.poll()
            for topic_info, message_dict in consumedata.items():
                print(f"poll message size : {len(message_dict)}")
                futures_list = []
                start_tm = time.time()
                with futures.ThreadPoolExecutor(max_workers=10) as executor:
                    for consume_message in message_dict:
                        future = executor.submit(self.setting_event, consume_message.value, ib_schedule, sb_schedule)
                        futures_list.append(future)
                    for future in futures.as_completed(futures_list):
                        result = future.result()
                        done = future.done()
                        cancelled = future.cancelled()
                        print(f"Future Result : {result}, Done : {done}, Cancelled : {cancelled}")
                end_tm = time.time() - start_tm
                print(f"Time : {end_tm:.5f}sec")
            threading.Timer(interval=1.0, function=self._start_consumer, args=(ib_schedule, sb_schedule)).start()
        except Exception as e:
            print(f"_start_consumer [ERROR] : {e}")

    def setting_event(self, value, ib_schedule, sb_schedule):
        print("setting event start")
        event = PredictionEvent(self.set_message_value(value))
        event.set_original_message(value)
